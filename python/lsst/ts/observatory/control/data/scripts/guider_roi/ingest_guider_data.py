...
Usage:
    # Summit environment (uses BestEffortIsr)
...
class GuiderDataIngester:
    ...
    def detect_butler_environment(self) -> Butler:
        ...
        if self.repo_path:
            ...
            if not BEST_EFFORT_ISR_AVAILABLE:
                raise RuntimeError(
                    "No --repo-path provided and BestEffortIsr not available. "
                    "Either provide --repo-path for local Butler or run in an "
                    "environment with BestEffortIsr."
                )
            logger.info("️  Using BestEffortIsr (summit environment)")
            return self._create_besteffort_butler()

    def _create_besteffort_butler(self) -> Butler:
        """Create Butler using BestEffortIsr."""
        try:
            self.best_effort_isr = BestEffortIsr()
            butler = self.best_effort_isr.butler
            if butler is None:
                raise RuntimeError("BestEffortIsr.butler is None")
            self.use_besteffort = True
            logger.info(" BestEffortIsr Butler created successfully")
            return butler
        except Exception as e:
            raise RuntimeError(f"Failed to create BestEffortIsr Butler: {e}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "If not provided, uses BestEffortIsr (summit environment)",
    )
    parser.add_argument(
        "--collection",
