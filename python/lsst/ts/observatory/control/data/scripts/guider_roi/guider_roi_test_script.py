...
Usage:
    # Summit environment (uses BestEffortIsr Butler)
...
def check_dependencies() -> bool:
    ...
    try:
        from lsst.summit.utils import BestEffortIsr  # noqa: F401
        ...
        print("ERROR: BestEffortIsr not available")

    print(f"\n Mode: {'Summit' if best_effort else 'Local'} environment")
    return True


def check_butler_repository(
    repo_path: Optional[str], collection: str, catalog_dataset: str, nside: int = 32
) -> list[int]:
    ...
    if repo_path:
        ...
        print("\n Checking Butler repository: BestEffortIsr")
        try:
            ...
            if butler is None:
                print("ERROR: BestEffortIsr.butler is None")
                return []
        except Exception as e:
            print(f"ERROR: Failed to create BestEffortIsr Butler: {e}")
            return []
...
def setup_guider_rois_with_butler(
    ...
    """Setup GuiderROIs with appropriate Butler (BestEffortIsr or local).

    Parameters
    ----------
    repo_path : str, optional
        Path to local Butler repository. If None, uses BestEffortIsr.
    catalog_dataset : str
    ...
    if repo_path is None:
        print("\n Setting up GuiderROIs with BestEffortIsr...")
        try:
            groi = GuiderROIs(
            ...
            print(" Direct initialization successful (BestEffortIsr)")
            return groi
        except RuntimeError as e:
            print(f" ERROR: Failed to initialize GuiderROIs with BestEffortIsr: {e}")
            raise
    else:
        ...
        try:
            ...
            if "BestEffortIsr" in str(e):
                print(
                    " BestEffortIsr not available, using local Butler with mocking..."
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--repo-path",
        default=None,
        help="Path to local Butler repository. If not provided, uses BestEffortIsr (summit environment)",
    )
    parser.add_argument(
        "--collection",
    ...
    pixels = check_butler_repository(
        args.repo_path, args.collection, args.catalog_dataset, args.nside
    )
    if not pixels:
        ...
        if args.repo_path:
            ...
            print("Please ensure data has been ingested to the BestEffortIsr Butler")
        sys.exit(1)
    ...
    if args.repo_path:
        ...
        print("Repository: BestEffortIsr Butler")
