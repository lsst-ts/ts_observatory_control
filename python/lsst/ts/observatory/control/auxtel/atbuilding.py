...
class ATBuilding(RemoteGroup):
    ...
    def usages(self) -> typing.Dict[int, UsagesResources]:
        if self._usages is None:
            ...
            usages[self.valid_use_cases.StandardStateTransition] = UsagesResources(
                components_attr=["atbuilding"],
                readonly=False,
                generics=["summaryState", "heartbeat"],
            )
    ...
    async def _wait_for_vent_gate_state(
        ...
        while pending:
            try:
                vent_gate_state = await self.rem.atbuilding.evt_ventGateState.next(
