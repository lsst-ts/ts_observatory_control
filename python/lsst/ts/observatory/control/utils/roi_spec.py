...
class ROICommon(BaseModel):
    ...
    def check_rows(cls, v: int) -> int:
        if v < 0:
            raise ValueError(f"Minimum rows must be positive, not {v!r}.")
        return v

    @validator("cols")
    def check_cols(cls, v: int) -> int:
        if v < 0:
            raise ValueError(f"Minimum cols must be positive, not {v!r}.")
        return v
    ...
    def check_integrationTimeMillis(cls, v: int) -> int:
        if v < 0:
            raise ValueError(
                f"Minimum integration_time_millis must be positive, not {v!r}."
            )
        return v
